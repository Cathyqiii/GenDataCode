...
class CurLearn(object):
    def __init__(self, scaler, model, args, train_set, test_set, heldout_set, directory=None,
    ...
    def train_model_batches(self,order,num_batches, batch_size=100,
        ...
        for batch in range(num_batches):
            ...
            batch_loss = self.train_on_batch('sadc',batch, data, target)
    ...
    def train_on_batch(self,ktype, batch, data, target):
        self.model.train()
        ...
        if ktype == 'sadc':
            ...
            for i in range(d_len):
                ...
                #self.loss_fun(output.squeeze().cuda(), target)
                b_item_loss = self.loss_fun(output[i, :].cuda(), target[i, :])
                b_item_loss.backward(retain_graph=True)
                batch_loss.append(b_item_loss.detach().cpu().item())
                '''
                for dd in self.model.parameters():
                    print(dd.grad.data)
                    break;
                '''
            ...
            loss = torch.tensor(batch_loss).sum()
