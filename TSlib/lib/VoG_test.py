...
def weights_init(m):
    if isinstance(m, torch.nn.Conv3d):
        torch.nn.init.xavier_uniform_(m.weight.data, torch.nn.init.calculate_gain('relu'))
...
def plot_decision_boundaries(X, y, model):
    ...
    # np.meshgrid() 是 NumPy 库中的一个函数，用于生成 N 维坐标网格。其作用是将一维数组转换为二维坐标数组，以便进行向量化运算。
    # Predictions to obtain the classification results
    Z = torch.argmax(torch.nn.Softmax(dim=1)(model(torch.FloatTensor(np.c_[xx.ravel(), yy.ravel()]))),
                     dim=1).detach().numpy().reshape(xx.shape)
...
for ii in range(grad_X.shape[0]):
    ...
    training_class_variances[int(grad_Y[ii].item())].append(
        np.mean(np.sqrt(sum([(mm - mean_grad) ** 2 for mm in temp_grad]) / len(temp_grad))))
...
plt.figure()
plt.scatter(np.array(normalized_vog)[grad_Y == 0], np.array(distance_from_boundary)[grad_Y == 0], linewidths=1.5,
            edgecolors='k', c='b', alpha=0.6)
plt.scatter(np.array(normalized_vog)[grad_Y == 1], np.array(distance_from_boundary)[grad_Y == 1], linewidths=1.5,
            edgecolors='k', c='r', alpha=0.6)
