...
class DShap(object):
    ...
    def one_iteration(self, model, tolerance, sources=None):
        ...
        # x_stack = torch.zeros((1,) + tuple(self.train_set[0][0].shape))#行列
        # y_stack = torch.zeros((1,),dtype= int)#行列
        x_stack = torch.zeros((0, self.train_set[1][0].shape[1], self.train_set[1][0].shape[2]))  # 行列
        y_stack = torch.zeros((0, self.train_set[1][1].shape[1]))  # 行列
    ...
    def _portion_performance_series(self, idxs, scaler):
        ...
        trainer = Trainer('sadc', self.model, loss, optimizer, train_loader, val_loader, self.test_set, scaler,
                          self.args, lr_scheduler=lr_scheduler)
