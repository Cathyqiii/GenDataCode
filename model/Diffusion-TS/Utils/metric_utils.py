...
def visualization(ori_data, generated_data, analysis, compare=3000, save_path=None):
    ...
    Args:
      ...
      - save_path: path to save the figure.
                   If it ends with a common image extension, treated as file path.
                   Otherwise treated as directory, and file named f"{analysis}.png" will be created inside.
    ...
    if save_path is not None:
        ...
        image_exts = {'.png', '.jpg', '.jpeg', '.pdf', '.svg', '.eps'}
        ext = os.path.splitext(save_path)[1].lower()
        if ext in image_exts:
            # save_path is already a file path
            final_save_path = save_path
            dirname = os.path.dirname(final_save_path)
            if dirname:
                os.makedirs(dirname, exist_ok=True)
        else:
            # treat save_path as directory, create file name from analysis
            dirname = save_path
            os.makedirs(dirname, exist_ok=True)
            final_save_path = os.path.join(dirname, f"{analysis}.png")
    ...
    if analysis == 'pca':
        ...
        if final_save_path is not None:
            plt.savefig(final_save_path, dpi=100, bbox_inches='tight')
        plt.show()
