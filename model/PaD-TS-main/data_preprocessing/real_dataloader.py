...
from model.TimeGAN.lib.data import real_data_loading
...
class CustomDataset(Dataset):
    def __init__(
        ...
        self.rawdata = real_data_loading(self.name, 32)
        ...
        self.samples = train if period == "train" else test
        if period == "test":
        ...
        # data = train if period == "train" else test
        #
        # # 用 window 切滑动窗口
        # xs = []
        ...
        #
        # self.samples = np.stack(xs, axis=0)  # (N, 24, 7)
        # self.sample_num = self.samples.shape[0]
