...
# 当前文件：D:\GenDataCode\model\TimeGAN\lib\timegan.py
current_dir = os.path.dirname(os.path.abspath(__file__))  # D:\GenDataCode\model\TimeGAN\lib
timegan_dir = os.path.dirname(current_dir)  # D:\GenDataCode\model\TimeGAN
model_dir = os.path.dirname(timegan_dir)  # D:\GenDataCode\model
project_root = os.path.dirname(model_dir)  # D:\GenDataCode

# 将所有需要的路径都添加到 sys.path
sys.path.append(project_root)  # 添加根目录，用于找到 TSlib
sys.path.append(timegan_dir)  # 添加 TimeGAN 目录，用于找到 utils.py
sys.path.append(current_dir)  # 添加当前目录，用于找到 data.py 和 modules.py
...
# 直接从 timegan_dir 导入 utils
from utils import extract_time, random_generator, NormMinMax
from data import batch_generator
from modules import Encoder, Recovery, Generator, Discriminator, Supervisor  # 从当前目录导入 modules.py
...
class BaseModel:
    ...
    def __init__(self, opt, ori_data):
        ...
        self.root_path = os.path.abspath(os.path.join(_current_dir, '..'))

        # Initalize variables.
        self.opt = opt
        self.ori_data = ori_data

        if self.opt.batch_size > len(self.ori_data):
            self.opt.batch_size = len(self.ori_data)

        self.ori_time, self.max_seq_len = extract_time(self.ori_data)
        self.data_num, _, _ = np.asarray(ori_data).shape  # 3661; 24; 6
        self.trn_dir = os.path.join(self.root_path, self.opt.outf, self.opt.data_name, "train")
        self.tst_dir = os.path.join(self.root_path, self.opt.outf, self.opt.data_name, "test")
    ...
    def save_weights(self, epoch):
        ...
        修改：权重文件路径和npy文件路径统一（保存在data_name目录下的weights子文件夹）
        Args:
            epoch ([int]): Current epoch number.
        """
        # 核心修改：将权重路径改为基于 opt.data_name（和npy文件同目录）
        weight_dir = os.path.join(self.root_path, self.opt.outf, self.opt.data_name, "weights")
    ...
    def save_data(self):
        """保存生成的npy文件（路径逻辑不变）"""
        data_dir = os.path.join(self.root_path, self.opt.outf, self.opt.data_name)
        if not os.path.exists(data_dir):
            os.makedirs(data_dir)

        print("generated data shape:", np.array(self.generated_data).shape)
        np.save(f'{data_dir}/{self.opt.data_name}.npy', np.array(self.generated_data))
    ...
    def train(self):
        ...
        # self.generated_data = self.generation(self.ori_data.shape[0])
...
"""
    ...
    def generation(self, num_samples, mean=0.0, std=1.0):
        ...
        # self.X0, self.T = batch_generator(
        #     self.ori_data, self.ori_time, num_samples
        # )
        self.Z = random_generator(
            self.opt.batch_size, self.opt.z_dim, self.T, self.max_seq_len)
