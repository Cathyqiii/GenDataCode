...
# AirQuality 数据集配置
input_file = r'd:\useless\研究生\时间序列生成与增强\数据处理\AirQuality(Italian)\data\AirQualityUCI_final.xlsx'
output_dir = r'd:\useless\研究生\时间序列生成与增强\数据处理\AirQuality(Italian)\output'
os.makedirs(output_dir, exist_ok=True)

# 所有数据特征（跳过Date和Time）
all_features = ['CO(GT)', 'PT08.S1(CO)', 'C6H6(GT)', 'PT08.S2(NMHC)', 
                'NOx(GT)', 'PT08.S3(NOx)', 'NO2(GT)', 'PT08.S4(NO2)', 
                'PT08.S5(O3)', 'T', 'RH', 'AH', 'CO_IAQI', 'NO2_IAQI', 'AQI']

# 可选参数
candidate_k = range(2, 8)  # 候选K值
selected_k = None  # 如果为None，则使用最优k；否则使用指定的k值
clustering_features = None  # 如果为None，则使用所有特征；否则指定使用的特征列表
...
def run_clustering(selected_features=None, selected_k_value=None):
    ...
    """
    print("=" * 80)
    print("使用KMeans进行AirQuality数据聚类分析")
    print("=" * 80)
...
def main():
    print("\n" + "=" * 80)
    print("AirQuality 数据集聚类分析")
    print("=" * 80)
    
    # 配置参数
    # 可选：指定用于聚类的特征（默认使用所有特征）
    # selected_features_for_clustering = None  # 使用所有特征
    selected_features_for_clustering = ['T', 'RH', 'AH', 'AQI']  # 指定特征

    # 可选：指定K值（默认使用最优K）
    # selected_k_value = None  # 自动计算最优K
    selected_k_value = 3  # 自动计算最优K
    # 运行聚类
